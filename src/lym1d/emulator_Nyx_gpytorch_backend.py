...
class ExactGPModel(gpytorch.models.ExactGP):
    def __init__(self, train_x, train_y, likelihood, kerneltype='SE', smooth_lengths=1.0, sigma_l=None, sigma_0=None):
        ...
        if kerneltype == 'SE':
            self.covar_module = gpytorch.kernels.ScaleKernel(gpytorch.kernels.RBFKernel(lengthscale=smooth_lengths,ard_num_dims=train_x.shape[1]))
        elif kerneltype == 'M52':
            self.covar_module = gpytorch.kernels.ScaleKernel(gpytorch.kernels.MaternKernel(nu=2.5, lengthscale=smooth_lengths,ard_num_dims=train_x.shape[1]))
        elif kerneltype == 'M32':
            self.covar_module = gpytorch.kernels.ScaleKernel(gpytorch.kernels.MaternKernel(nu=1.5, lengthscale=smooth_lengths,ard_num_dims=train_x.shape[1]))
