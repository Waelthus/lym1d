...
class ExactGPModel(gpytorch.models.ExactGP):
    def __init__(self, train_x, train_y, likelihood, kerneltype='SE', smooth_lengths=1.0, sigma_l=None, sigma_0=None):
        ...
        if kerneltype == 'SE':
            ...
            self.covar_module = gpytorch.kernels.ScaleKernel(gpytorch.kernels.MaternKernel(nu=1.5, lengthscale=torch.tensor(smooth_lengths),ard_num_dims=train_x.shape[1])+ (sigma_l**2 * gpytorch.kernels.LinearKernel() if sigma_l is not None else 0)) + (sigma_0**2 * gpytorch.kernels.ConstantKernel() if sigma_0 is not None else 0)
...
def create_emulator(par_grid, stat_grid, smooth_lengths, noise=None, npc=5, optimize=True, output_cov=True,sigma_l=None,sigma_0=None, noPCA=False, kerneltype='SE',make_deriv_emulator=False):
    ...
    if noise is None:
        noise = torch.tensor(np.repeat(1e-5,par_grid.shape[0]), dtype=torch.float32)
        train_noise=True
    else:
        noise = torch.tensor(np.repeat(noise,par_grid.shape[0]), dtype=torch.float32)
        train_noise=False
    ...
    for i, w in enumerate(ww):
        ...
        likelihood = gpytorch.likelihoods.FixedNoiseGaussianLikelihood(noise=noise**2, learn_additional_noise=train_noise)
